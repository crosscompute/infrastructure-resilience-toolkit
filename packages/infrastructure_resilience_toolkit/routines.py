...
def compute_voltage_drop_percent(
    ...
    source_line_to_neutral_in_volts = get_line_to_neutral_in_volts(
        source_line_to_line_in_volts)
    # 1990 IEEE Standard 241 Page 72
    try:
        target_line_to_neutral_in_volts = sqrt(
            source_line_to_neutral_in_volts ** 2 - (
                current_in_amps * reactance_in_ohms * cosine_phi
                - current_in_amps * resistance_in_ohms * sine_phi
            ) ** 2) - (
                current_in_amps * resistance_in_ohms * cosine_phi
                + current_in_amps * reactance_in_ohms * sine_phi)
    except ValueError:
        target_line_to_neutral_in_volts = 0
    target_line_to_line_in_volts = get_line_to_line_in_volts(
        target_line_to_neutral_in_volts)
    line_to_line_drop_in_volts = (
        source_line_to_line_in_volts - target_line_to_line_in_volts)
